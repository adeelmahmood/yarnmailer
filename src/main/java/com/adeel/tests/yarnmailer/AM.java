...
import com.adeel.tests.yarnmailer.utils.ResourceUtils;
...
public class AM {
	...
	private String appName = "YarnMailer";
	...
	public boolean run() throws IOException, YarnException {
		...
		// make jar available for containers as well
		String suffix = appName + Path.SEPARATOR + appAttemptId.getApplicationId() + Path.SEPARATOR + containerJar.replace(".jar", "-conts.jar");
		Path dest = new Path(fs.getHomeDirectory(), suffix);
		fs.copyFromLocalFile(new Path(containerJar), dest);
		containerJarPath = dest.toString();
	...
	private ContainerRequest requestForContainer() {
		...
		log.info("Requesting container => " + containerMem + "MB, " + containerVCores + "VCores. "
				+ capability.toString());
	...
	static class NMCallbackHandler implements NMClientAsync.CallbackHandler {
		...
		public NMCallbackHandler(AM am) {
			this.am = am;
		}
	...
	private class LaunchContainerRunnable implements Runnable {
		...
		public void run() {
			...
			// // set classname
			vargs.add("-jar spring-conts.jar");
